...
# ── RNP — Registro Nacional de Proveedores (OSCE/OECE) ────

def rnp_consultar_ruc(ruc):
    """Consulta el RNP del OSCE por número de RUC."""
    try:
        url = f"https://portal.osce.gob.pe/rnp/content/consultas-en-linea-Sin-Clave-RNP?ruc={ruc}"
        req = urllib.request.Request(url, headers={
            "User-Agent":      "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
            "Accept":          "text/html,application/xhtml+xml",
            "Accept-Language": "es-PE,es;q=0.9",
            "Referer":         "https://portal.osce.gob.pe/rnp/",
        })
        with urllib.request.urlopen(req, timeout=15) as resp:
            html = resp.read().decode("utf-8", "replace")
            return _parsear_rnp_html(ruc, html)
    except Exception as e:
        print(f"[VIGIA] RNP error ruc {ruc}: {e}")
        return _rnp_fallback(ruc)


def rnp_consultar_nombre(nombre):
    """Busca en el RNP por nombre o razón social."""
    try:
        params = urllib.parse.urlencode({"razonSocial": nombre})
        url = f"https://portal.osce.gob.pe/rnp/content/consultas-en-linea-Sin-Clave-RNP?{params}"
        req = urllib.request.Request(url, headers={
            "User-Agent":      "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
            "Accept":          "text/html,application/xhtml+xml",
            "Accept-Language": "es-PE,es;q=0.9",
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            html = resp.read().decode("utf-8", "replace")
            return _parsear_rnp_html(None, html)
    except Exception as e:
        print(f"[VIGIA] RNP buscar nombre error: {e}")
        return []


def _parsear_rnp_html(ruc, html):
    """Extrae datos del HTML de respuesta del RNP."""
    import re
    resultados = []
    ...
    for fila in filas:
        ...
        if len(celdas) >= 3 and any(c for c in celdas if len(c) > 3):
            resultados.append({
                "fuente":        "RNP_REAL",
                "ruc":           celdas[0] if len(celdas) > 0 else ruc or "",
                "razon_social":  celdas[1] if len(celdas) > 1 else "",
                "estado":        celdas[2] if len(celdas) > 2 else "",
                "tipo":          celdas[3] if len(celdas) > 3 else "",
                "vigencia":      celdas[4] if len(celdas) > 4 else "",
                "inhabilitado":  "inhabili" in " ".join(celdas).lower(),
                "alerta_vigia":  _analizar_alerta_rnp(celdas),
            })
    if not resultados and ruc:
        # Intentar extracción directa si solo hay un resultado
        ruc_found   = re.search(r"\b" + str(ruc) + r"\b", html)
        estado_found = re.search(r"ACTIVO|SUSPENDIDO|INHABILITADO|NULO", html, re.IGNORECASE)
        if ruc_found or estado_found:
            estado = estado_found.group(0).upper() if estado_found else "DESCONOCIDO"
            nombre_match = re.search(r"<td[^>]*>([A-Z][A-Z\s\.&]{5,60})</td>", html)
            resultados.append({
                "fuente":       "RNP_REAL",
                "ruc":          ruc or "",
                "razon_social": nombre_match.group(1).strip() if nombre_match else "",
                "estado":       estado,
                "inhabilitado": estado in ("SUSPENDIDO", "INHABILITADO", "NULO"),
                "alerta_vigia": ["🔴 INHABILITADO para contratar con el Estado"] if estado in ("SUSPENDIDO", "INHABILITADO") else ["✅ Proveedor activo en RNP"],
            })
    return resultados


def _rnp_fallback(ruc):
    """Retorna resultado indicando que no se pudo consultar."""
    return [{
        "fuente":       "RNP_SIN_CONEXION",
        "ruc":          ruc or "",
        "razon_social": "",
        "estado":       "NO_CONSULTADO",
        "inhabilitado": False,
        "alerta_vigia": ["⚠️ No se pudo conectar al RNP. Consulta manualmente en portal.osce.gob.pe/rnp"],
    }]


def _analizar_alerta_rnp(celdas):
    """Genera alerta según estado del proveedor en RNP."""
    texto = " ".join(celdas).lower()
    if "inhabili" in texto:
        return ["🔴 CRÍTICO: Proveedor INHABILITADO para contratar con el Estado"]
    if "suspend" in texto:
        return ["🟠 ALERTA: Proveedor SUSPENDIDO en RNP"]
    if "nulo" in texto:
        return ["🔴 CRÍTICO: Inscripción RNP declarada NULA — posible documentación falsa"]
    if "activo" in texto:
        return ["✅ Proveedor activo y habilitado en RNP"]
    return ["⚠️ Estado RNP no determinado — verificar manualmente"]


def rnp_cruzar_con_caso(caso_id):
    """Cruza los datos de un caso VIGIA con el RNP automáticamente."""
    db  = con()
    cur = db.cursor()
    cur.execute("SELECT obra, seace, hallazgo FROM casos WHERE id=?", (caso_id,))
    row = cur.fetchone()
    db.close()
    if not row:
        return {"ok": False, "msg": "Caso no encontrado"}
    obra, seace, hallazgo = row
    # Buscar empresa mencionada en el hallazgo o seace
    resultados_rnp = rnp_consultar_nombre(seace) if seace else []
    alertas_criticas = [r for r in resultados_rnp if r.get("inhabilitado")]
    return {
        "ok":              True,
        "caso_id":         caso_id,
        "obra":            obra,
        "proveedores":     resultados_rnp,
        "alertas_criticas": alertas_criticas,
        "resumen":         f"{len(alertas_criticas)} proveedor(es) inhabilitado(s) encontrado(s)" if alertas_criticas else "Sin inhabilitaciones detectadas",
    }
    ...
    def do_GET(self):
        ...
        try:
            if path == "/api/casos":
                ...
                caso_id = qs.get("caso","")[0] if qs.get("caso") else ""
                if caso_id:
                    res = rnp_cruzar_con_caso(caso_id)
                    self.jout(res)
                elif ruc:
                    res = rnp_consultar_ruc(ruc)
                    self.jout({"ok":True,"total":len(res),"resultados":res,
                               "fuente":"RNP_OSCE"})
                elif nombre:
                    res = rnp_consultar_nombre(nombre)
                    self.jout({"ok":True,"total":len(res),"resultados":res,
                               "fuente":"RNP_OSCE"})
                else:
                    self.jout({"ok":False,"msg":"Proporciona ruc, nombre o caso"})
